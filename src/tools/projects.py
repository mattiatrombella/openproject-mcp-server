...
class CopyProjectInput(BaseModel):
    """Input model for copying a project from a template/source project."""
    source_project_id: int = Field(
        ..., description="ID of the project (or template) to copy from", gt=0
    )
    name: str = Field(..., description="Name of the new project", min_length=1, max_length=255)
    identifier: str = Field(
        ...,
        description="Identifier of the new project (lowercase, no spaces)",
        min_length=1,
        max_length=100,
    )
    copy_work_packages: bool = Field(True, description="Copy work packages")
    copy_members: bool = Field(True, description="Copy project members")
    copy_versions: bool = Field(True, description="Copy versions/milestones")
    copy_categories: bool = Field(True, description="Copy work package categories")
    copy_wiki: bool = Field(True, description="Copy the wiki")
    copy_forums: bool = Field(True, description="Copy forums")
    copy_queries: bool = Field(True, description="Copy saved views/queries")
    copy_boards: bool = Field(True, description="Copy boards")
    copy_overview: bool = Field(True, description="Copy the project overview dashboard")
    send_notifications: bool = Field(
        False, description="Send email notifications for the copy operation"
    )
    extra_meta: Optional[dict] = Field(
        None,
        description=(
            "Advanced: extra _meta copy flags to pass through verbatim "
            "(camelCase keys), merged over the defaults above"
        ),
    )


@mcp.tool
async def copy_project(input: CopyProjectInput) -> str:
    """Create a new project by copying an existing project or template.

    Use this to spin up a project from a template (any project flagged as a
    template in OpenProject, or any existing project). The copy flags control
    which associated data is carried over; all default to True for a full
    template clone. Categories are copied as part of the source project — the
    OpenProject API v3 has no standalone "create category" endpoint, so copying
    a template is the supported way to seed categories into a new project.

    The copy runs as an asynchronous background job; this tool returns the job
    status. Use get_project on the new identifier once the job completes.

    Args:
        input: source_project_id, name, identifier and optional copy flags

    Returns:
        Success message with the copy job status

    Example:
        {
            "source_project_id": 12,
            "name": "Acme Onboarding",
            "identifier": "acme-onboarding",
            "copy_work_packages": false
        }
    """
    try:
        client = get_client()
        ...
        meta = {
            "copyWorkPackages": input.copy_work_packages,
            "copyMembers": input.copy_members,
            "copyVersions": input.copy_versions,
            "copyCategories": input.copy_categories,
            "copyWiki": input.copy_wiki,
            "copyForums": input.copy_forums,
            "copyQueries": input.copy_queries,
            "copyBoards": input.copy_boards,
            "copyOverview": input.copy_overview,
            "sendNotifications": input.send_notifications,
        }
        if input.extra_meta:
            meta.update(input.extra_meta)

        result = await client.copy_project(
            input.source_project_id, input.name, input.identifier, meta
        )

        text = format_success("Project copy started!\n\n")
        text += f"**New project**: {input.name} ({input.identifier})\n"
        text += f"**Copied from**: project #{input.source_project_id}\n"

        # Copy is async; the response is usually a JobStatus resource.
        status = result.get("status")
        if status:
            text += f"**Job status**: {status}\n"
        message = result.get("message")
        if message:
            text += f"**Details**: {message}\n"

        # If the job already resolved to a project, surface it.
        project_link = result.get("_links", {}).get("project")
        if isinstance(project_link, dict) and project_link.get("href"):
            new_id = project_link["href"].rstrip("/").split("/")[-1]
            text += f"**Created project ID**: #{new_id}\n"
        else:
            text += (
                "\nThe copy runs in the background. "
                "Use get_project or list_projects to confirm completion.\n"
            )

        return text

    except Exception as e:
        return format_error(f"Failed to copy project: {str(e)}")
